...
const EditCustomerFormDiv =  motion(styled.div`
    @media ${StyledMediaQuery.XS} {
      position: fixed;
      right: 0;
      top: 0;
...
const EditCustomerFormContent = styled.div`
    @media ${StyledMediaQuery.XS} {
        display: flex;
        flex-direction: column;
        gap: 24px;
...
const EditCustomerFormHeading = styled.div`
    @media ${StyledMediaQuery.XS} {
        display: flex;
...
const EditCustomerForm: React.FC<EditCustomerFormProps> = ({ onClose, customer }) => {
  const [newCustomerFirstName, setNewCustomerFirstName] = useState<string>(customer.firstName || '');
  const [newCustomerLastName, setNewCustomerLastName] = useState<string>(customer.lastName || '');
  const [newCustomerPhoneNumber, setNewCustomerPhoneNumber] = useState<string>(formatPhoneNumber(customer.fullPhoneNumber) || '');
...
useEffect(() => {
    setNewCustomerFirstNameValid(!!newCustomerFirstName);
    setNewCustomerPhoneNumberValid(isValidPhoneNumber(newCustomerPhoneNumber));
  }, [newCustomerFirstName, newCustomerPhoneNumber]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!isValidPhoneNumber(newCustomerPhoneNumber)) {
    ...
    const payload = {
      customerDetails: {
        newFirstName: newCustomerFirstName,
        newLastName: newCustomerLastName,
          ...
          <form onSubmit={handleSubmit}>
            <FormFields>
                <InputField
                    ...
                    onChange={(value) => {
                    setNewCustomerFirstName(value);
                    setNewCustomerFirstNameValid(!!value); // Validate based on whether the input is not empty
                    }}
                    required={true}
                    placeholder='Ex: John'
                />
                ...
                <InputField
                    ...
                    setNewCustomerPhoneNumber(formattedNumber);
                    setNewCustomerPhoneNumberValid(isValidPhoneNumber(formattedNumber));
                    }}
                    required={true}
                ...
                <BirthdayDiv>
                    ...
                    <BirthdayFieldsDiv>
                        <DropdownField 
                            ...
                            onChange={(value) => setNewCustomerBirthdayMonth(value)}
                            label="" 
                            useDefaultDropdown={false} 
                            options={birthdayMonthOptions}
                        />
                        <DropdownField 
                            value={newCustomerBirthdayDay} 
                            onChange={(value) => setNewCustomerBirthdayDay(value)}
                            label="" 
                            useDefaultDropdown={false} 
                            options={birthdayDayOptions}
                        />
                        <DropdownField 
                            value={newCustomerBirthdayYear} 
                            onChange={(value) => setNewCustomerBirthdayYear(value)}
                            label="" 
                            useDefaultDropdown={false} 
            ...
            <Button
                  buttonTypeVariant={
                    isNewCustomerFirstNameValid && isNewCustomerPhoneNumberValid /* && Add conditions for other fields */
                      ? 'primary'
                      : 'disabled'
                  }
                  sizeVariant='large'
                  ...
                  type='submit'
                  disabled={
                    !isNewCustomerFirstNameValid ||
                    !isNewCustomerPhoneNumberValid /* || Add conditions for other fields */
                  }
                />
        </form>
      </FormAndButton>
